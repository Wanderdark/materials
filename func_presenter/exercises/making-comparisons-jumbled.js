const makingComparisonsJumbledPool = [
...
let makingComparisonsJumbledRemaining = [];

function shuffleMakingComparisonsJumbled(items) {
  const copy = [...items];
...
function buildMakingComparisonsJumbledBank() {
  return makingComparisonsJumbledPool.map(([sentence, imagePath], index) => ({
    id: `making-comparisons-jumbled-${index + 1}`,
    sentence,
    imagePath,
    type: "STATEMENT",
...
const makingComparisonsJumbledExercise = window.exerciseActivityModules.jumbledSentences({
  id: "making-comparisons-jumbled",
  ...
  reset() {
    makingComparisonsJumbledRemaining = shuffleMakingComparisonsJumbled(buildMakingComparisonsJumbledBank());
  },
  hasMoreQuestions() {
    return makingComparisonsJumbledRemaining.length > 0;
  },
  buildQuestions() {
    return makingComparisonsJumbledRemaining.splice(0, 10);
  }
});

window.exerciseModules = window.exerciseModules || [];
window.exerciseModules.push(makingComparisonsJumbledExercise);
