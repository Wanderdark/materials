const makingComparisonsTrueFalseSets = [
  {
...
function buildMakingComparisonsTrueFalseStatements() {
  return shuffleActivityItems(makingComparisonsTrueFalseSets.flatMap((set, setIndex) => [
    ...set.trueStatements.map((sentence, index) => ({
      id: `makingComparisons-tf-${setIndex + 1}-t-${index + 1}`,
      sentence,
      isTrue: true,
      imagePath: set.imagePath
    })),
    ...set.falseStatements.map((sentence, index) => ({
      id: `makingComparisons-tf-${setIndex + 1}-f-${index + 1}`,
      sentence,
      isTrue: false,
      imagePath: set.imagePath
    }))
...
const makingComparisonsTrueFalseExercise = window.exerciseActivityModules.trueFalseGrid({
  id: "making-comparisons-true-false",
  title: "TRUE - FALSE",
  pageSize: 8,
  ...
  buildStatements() {
    return buildMakingComparisonsTrueFalseStatements();
  }
});

window.exerciseModules = window.exerciseModules || [];
window.exerciseModules.push(makingComparisonsTrueFalseExercise);
