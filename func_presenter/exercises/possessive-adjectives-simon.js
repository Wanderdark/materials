...
const possSimonBlinkOverlays = {
  oliver: { src: "images/possessive-adjectives/oliver_eyeshut.webp", x: 532, y: 79, width: 71, height: 24 },
  ingrid: { src: "images/possessive-adjectives/ingrid_eyeshut.webp", x: 1054, y: 119, width: 82, height: 35 }
};

function startPossessiveAdjectivesSimonSays(exercise, api) {
  ...
  function buildCommand() {
    ...
    const basePartList = state.difficulty === "basic" ? possSimonBodyPartsBasic : possSimonBodyPartsAdvanced;
    const availableParts = basePartList.concat(possSimonExtraBodyParts[target] || []);
  ...
  function pickVoice(gender) {
    ...
    return voices.find((voice) => pattern.test(voice.name) && !opposite.test(voice.name)) || voices[0];
  }
  ...
  function speakCommand(command, onEnd) {
    ...
    utterance.pitch = isOliver ? .4 : 1;
    const voice = pickVoice(isOliver ? "male" : "female");
  ...
  function drawHitAreas() {
    ...
    Object.entries(possSimonHotspots).forEach(([character, parts]) => {
      Object.entries(parts).forEach(([part, area]) => {
        ...
        const shape = document.createElementNS(NS, isPolygon ? "polygon" : isRect ? "rect" : "circle");
        if (isPolygon) {
  ...
  winExitButton.onclick = exitToSetup;
  exitButton.onclick = exitToSetup;

  api.hideAllScreens();
  $("possSimonGradeLabel").textContent = `GRADE ${api.state.grade}`;
  drawHitAreas();
  setupBlinking();
  resetGame();
...
const possessiveAdjectivesSimonExercise = {
  id: "possessive-adjectives-simon-says",
  title: "SIMON SAYS: CLICK IT!",
  activity: "possessive-adjectives-simon-says",
  supports(functionModule) {
    return functionModule.id === "possessive-adjectives";
  }
};

window.startPossessiveAdjectivesSimonSays = startPossessiveAdjectivesSimonSays;
window.exerciseModules = window.exerciseModules || [];
window.exerciseModules.push(possessiveAdjectivesSimonExercise);
