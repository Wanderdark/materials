const makingComparisonsChoiceSets = [
  {
...
window.makingComparisonsChoiceSets = makingComparisonsChoiceSets;

let makingComparisonsChoiceRemaining = [];

function shuffleMakingComparisonsChoiceItems(items) {
  const copy = [...items];
...
function buildMakingComparisonsQuestion(set, index) {
  const answer = shuffleMakingComparisonsChoiceItems(set.correct)[0];
  const distractors = shuffleMakingComparisonsChoiceItems(set.wrong).slice(0, 2);
  return {
    id: `making-comparisons-choice-${index + 1}`,
    referenceType: "CHOOSE THE CORRECT OPTION",
    prompt: "Choose the correct sentence.",
    imagePath: set.imagePath,
    options: shuffleMakingComparisonsChoiceItems([answer, ...distractors]),
    answer,
    instruction: "Look at the picture and choose the correct comparison."
  };
}

const makingComparisonsChoiceExercise = window.exerciseActivityModules.choice({
  id: "making-comparisons-choice",
  title: "CHOOSE CORRECT OPTION",
  compactPrompt: true,
  wideOptions: true,
  repeatable: true,
  supports(functionModule) {
    return functionModule.id === "making-comparisons";
  },
  reset() {
    makingComparisonsChoiceRemaining = shuffleMakingComparisonsChoiceItems(makingComparisonsChoiceSets);
  },
  hasMoreQuestions() {
    return makingComparisonsChoiceRemaining.length > 0;
  },
  buildQuestions() {
    if (!makingComparisonsChoiceRemaining.length) return [];
    return makingComparisonsChoiceRemaining.splice(0, 10).map(buildMakingComparisonsQuestion);
  }
});

window.exerciseModules = window.exerciseModules || [];
window.exerciseModules.push(makingComparisonsChoiceExercise);
